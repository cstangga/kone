/**
 * create.html 포함
 * 
 */

document.addEventListener('DOMContentLoaded', function() {
	$(document).ready(
		function() {
			// 서명 초기화
			$("#signature1").jSignature({
				'background-color': 'white',
				'height': '250px',
				'width': '714px',
				'lineWidth': 12,
			});

			$("#signature1").css({
				'border': '1px solid gray',
				'display': 'flex',
				'justify-content': 'center',
				'align-items': 'center',
				'margin': '25px'

			});

			// 서명하기 버튼 클릭 시 모달 표시
			$("#signbtn1").click(function(e) {
				e.preventDefault();
				$("#signModal1").modal('show');
			});

			// 다시 서명 버튼
			$("#reset1").click(function(e) {
				e.preventDefault();
				$("#signature1").jSignature("reset");
			});

			// 서명 저장 버튼
			$("#save1").click(
				function(e) {
					e.preventDefault();

					if ($("#signature1").jSignature("getData", "native").length === 0) {
						alert("서명을 해주세요.");
						return;
					}

					// 서명 데이터를 PNG 형식으로 가져오기
					let imgData = $("#signature1").jSignature("getData", "image");
					let imgBase64 = "data:" + imgData[0] + "," + imgData[1];

					// 이미지 표시 및 버튼 숨기기
					$("#signatureImg1").attr("src", imgBase64).show();
					//$("#signbtn1").hide();
					$("#signModal1").modal('hide');
					$("#signature1Data").val($("#signatureImg1").attr("src"));

					$("#signature1").jSignature("reset");
				});


			$("#signature2").jSignature({
				'background-color': 'white',
				'height': '250px',
				'width': '714px',
				'lineWidth': 12,
			});

			$("#signature2").css({
				'border': '1px solid gray',
				'display': 'flex',
				'justify-content': 'center',
				'align-items': 'center',
				'margin': '25px'

			});

			// 서명하기 버튼 클릭 시 모달 표시
			$("#signbtn2").click(function(e) {
				e.preventDefault();
				$("#signModal2").modal('show');
			});


			// 다시 서명 버튼
			$("#reset2").click(function(e) {
				e.preventDefault();
				$("#signature2").jSignature("reset");
			});

			// 서명 저장 버튼
			$("#save2").click(
				function(e) {
					e.preventDefault();

					if ($("#signature2").jSignature("getData", "native").length === 0) {
						alert("서명을 해주세요.");
						return;
					}

					// 서명 데이터를 PNG 형식으로 가져오기
					let imgData = $("#signature2").jSignature("getData", "image");
					let imgBase64 = "data:" + imgData[0] + "," + imgData[1];

					// 이미지 표시 및 버튼 숨기기
					$("#signatureImg2").attr("src", imgBase64).show();
					//$("#signbtn2").hide();
					$("#signModal2").modal('hide');
					$("#signature2Data").val($("#signatureImg2").attr("src"));

					$("#signature2").jSignature("reset");
				});

		});

		const file1 = document.getElementById('accuntfile');
		file1.addEventListener('change', function(e) {
			const fileName = e.target.files[0].name;
			const accountfileName = document.getElementById('accuntfileName');
			accountfileName.value = fileName;
		})

		const file2 = document.getElementById('cardfile');
		file2.addEventListener('change', function(e) {
			const fileName = e.target.files[0].name;
			const cardfileName = document.getElementById('cardfileName');
			cardfileName.value = fileName;
		})
})